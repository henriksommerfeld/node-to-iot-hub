...
/**
 * Copy files needed for running in production to 'dist' folder.
 * After this task you can copy the 'dist' folder to another machine, run 'npm install --production' 
 * and finally 'node server.js' to run the application. 
 * @return {Stream}
 */
gulp.task('build-production', ['typescript-compile', 'copy:package.json'], function() {
  return gulp;
});

gulp.task('copy:package.json', ['clean:generated'], function() {
  return gulp
    .src('package.json')
    .pipe(gulp.dest('dist/'));
});
